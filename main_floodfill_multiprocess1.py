'''基于main_multiprocess1.py的代码进行修改'''
import sys
# import random
import numpy as np
        ...
        for i in range(self.num+1, robot_num):
            if abs(robot_list[i].x - self.x) + abs(robot_list[i].y - self.y) == 2:
                if self.get_next_coordinate() == robot_list[i].get_next_coordinate(): # 如果即将发生碰撞
                    ...
                    # if robot_list[i].x == self.x: # 如果是垂直对撞情况
                    #     if gbmap.is_available(self.x + 1, self.y, self) or gbmap.is_available(self.x - 1, self.y, self): # 检查两侧是否有空间避让
                    #         return 1 # 等待即可 先停止，转换成距离为1的情况，然后水平方向上等待
                    #     else:
                    #         self.move_back_along_path()
                    #         return 2 # 
                    # elif robot_list[i].y == self.y: # 如果是水平对撞情况
                    #     if gbmap.is_available(self.x, self.y + 1, self) or gbmap.is_available(self.x, self.y - 1, self): # 检查两侧是否有空间避让
                    #         return 1 # 等待即可 先停止，转换成距离为1的情况，然后水平方向上等待
                    #     else:
                    #         self.move_back_along_path()
                    #         return 3 # 先停止，转换成距离为1的情况，然后垂直方向上等待
...
def path_thread(input_queue, output_queue, start_event):

    while True:
        # start_event.wait()
        try:
            start, end, planner, robot_number = input_queue.get(timeout=0.5)
            ...
            if start == end :
                ...
                path = planner.astar((start[0], start[1]),(end[0], end[1]))
...
if __name__ == "__main__":
    ...
    p = multiprocessing.Process(target=path_thread, args=(input_queue, output_queue,start_event,))
    p.start() 
    ...
    for zhen in range(1, 15001):
        ...
        if now_frame_id == 1:
            ...
            #     for i in range(200):
            #         for j in range(200):
            #             print(robot[k].reachable_region[i][j],end=' ',file = print_log)
            #         print('\n',file = print_log)
            #     print('\n',file = print_log)
            #     print_log.close()
        ...
        for i in range(robot_num):
                            ...
                            if (robot[i].x,robot[i].y) in robot[i].path_list:
                                ...
                                if len(robot[i].path_list)-1 == ind: # 如果已经走到终点
                                    ...
                                    if not input_queue.empty():
                                        ...
                                        input_queue.put(((robot[i].x, robot[i].y), (robot[i].mbx, robot[i].mby), planner, i))
            ...
            lock.release() # 结束后要释放锁

            if (robot[i].mbx, robot[i].mby) == (robot[i].x, robot[i].y):
                 robot[i].path_list = []
        ...
        # 充分利用时间
        end_zhen_time = time.time()
        execution_time = end_zhen_time - start_zhen_time
        if(execution_time < 0.01):
            time.sleep(0.01 - execution_time)
