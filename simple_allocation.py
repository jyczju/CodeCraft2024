...
def Robot_task(robot, gbmap, berth, num, now_frame_id, boat, gds_add_num, allocation_aid):  # 读取货物位置，为机器人分配搬运(get)任务
    ...
    for robot_x in robot:
        if robot_x.allocation_status == 0 and robot_x.status == 1:  # 还没有给出规划命令
            ...
            if robot_x.goods == 0:  # 执行get任务
                if len(gbmap.gds_dict) >= 20:
                    ...
                    for best_x in best:
                        ...
                        if robot_x.reachable_region[best_x[0]][best_x[1]] != 0: # 如果该货物处于可达区域
                            robot_x.mbx = best_x[0]
                            robot_x.mby = best_x[1]
                            # 更新分配任务的状态
                    ...
                    if find_flag >= len(best): # 最近的三个货物都不可达
            ...
            # with open('gds.txt', 'a') as f:
            #     # 将列表中的每个元素转换为字符串并写入文件
            #     f.write(str(now_frame_id) + '\n')
            #     i = 0
            #     while i<10:
            #         f.write(str(berth[i].now_gds_num) + '\n')
            #         i = i + 1
            #     f.write('\n')
...
def boat_task(berth, boat, now_frame_id, allocation_aid, boat_capacity):  # 轮船任务分配
    if now_frame_id == 1 :  # 第一帧先分配5个berth，按照运输时间短和位置分散两个因素来决定。
        ...
        while i < 8:
            if (berth[i].transport_time < berth[i + 1].transport_time):
                ...
                allocation_aid.berth_used.update({boat[j].num:berth[i].num})      # 记录使用的泊位
                allocation_aid.berth_sum[berth[i].num] = allocation_aid.berth_sum[berth[i].num] + 1 #统计港口去过的次数
            else:
                boat[j].ship(berth[i + 1].num, now_frame_id)
                # allocation_aid.berth_boat_stop.append(berth[i + 1].num)  # 记录一下哪个港口即将有船
                allocation_aid.berth_used.update({boat[j].num:berth[i + 1].num})   # 记录使用的泊位
                allocation_aid.berth_sum[berth[i+1].num] = allocation_aid.berth_sum[berth[i+1].num] + 1
        ...
        # allocation_aid.berth_boat_stop.append(berth[i + 1].num)  # 记录一下哪个港口即将有船
        allocation_aid.berth_used.update({boat[j].num:berth[9].num})   # 记录使用的泊位
        allocation_aid.berth_sum[berth[9].num] = allocation_aid.berth_sum[berth[9].num] + 1


    elif now_frame_id >= 13600:  # 时间快要结束了，直接开船 # 优化一下，直接开船的时间取决于船到达虚拟点的时间(已完成)
        for boat_x in boat:
        ...
        for boat_x in boat:
            ...
            if boat_x.status == 1 and boat_x.pos != -1:  # 处于泊位装货状态
                if now_frame_id - boat_x.start_id < 500:   #阈值内
                    ...
                    if boat_x.leaving_time == now_frame_id :
                        boat_x.go()
                        if boat_x.num in allocation_aid.berth_used:  # 删除之前先判断是否被删除过了
                            del allocation_aid.berth_used[boat_x.num]
                else:  #停时间太长了直接走
                    boat_x.go()
                    if boat_x.num in allocation_aid.berth_used:  # 删除之前先判断是否被删除过了
                        del allocation_aid.berth_used[boat_x.num]
                ...
                # berth_gds = {}
                # for berth_x in berth:
                #     if berth_x.num not in allocation_aid.berth_used.values():  #如果泊位空闲
                #         berth_gds.update({berth_x.num:berth_x.now_gds_num})
                #
                # max_key = max(berth_gds, key=berth_gds.get)  #找到货物最多的泊位

                while 1:
                    min_index = allocation_aid.berth_sum.index(min(allocation_aid.berth_sum))
                    if min_index not in allocation_aid.berth_used.values():  # 如果泊位空闲
                        break

                boat_x.ship(min_index, now_frame_id)
                allocation_aid.berth_used.update({boat_x.num: min_index})
                allocation_aid.berth_sum[min_index] = allocation_aid.berth_sum[min_index] + 1
    ...
    # if now_frame_id == 4000: #开始判断重点港口
    #     gds_z = []
    #     for berth_x in berth:
    #         gds_z.append(berth_x.now_gds_num)
    #         berth_x.now_gds_num = 0  #清零
    #     global gds_indices
    #     gds_indices = sorted(range(len(gds_z)), key=lambda i: gds_z[i], reverse=True)[:3]  #找到最常放的4个港口
    #
    # if now_frame_id == 8000:  # 开始判断重点港口
    #     for x in gds_indices:
    #         allocation_aid.berth_sum[x] = allocation_aid.berth_sum[x] - 1  #多去一次\
    #     gds_z = []
    #     for berth_x in berth:
    #         gds_z.append(berth_x.now_gds_num)
    #         berth_x.now_gds_num = 0  # 清零
    #     global gds_indices_1
    #     gds_indices_1 = sorted(range(len(gds_z)), key=lambda i: gds_z[i], reverse=True)[:3]  # 找到最常放的4个港口
    #
    # if now_frame_id == 12000:  # 开始判断重点港口
    #     for x in gds_indices_1:
    #         allocation_aid.berth_sum[x] = allocation_aid.berth_sum[x] - 1  #多去一次\
