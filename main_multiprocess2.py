...
def path_thread(input_queue, output_queue, end_event):

    while True:
        try:
            start, end, planner, robot_number = input_queue.get(timeout=0.2)
...
if __name__ == "__main__":
    ...
    p = multiprocessing.Process(target=path_thread, args=(input_queue, output_queue,end_event,))
    p.start() 
    ...
    for zhen in range(1, 15001):
        ...
        if(execution_time < 0.013):
            time.sleep(0.013 - execution_time)
