...
from simple_allocation_jyc import task_allocation_main
...
class Splitting_threads(object):
    ...
    def robot_move_thread(self):
        ...
        # string = str(ch[0])[1]
        # print(string)
        gbmap.init_map(ch) # 初始化地图 放到init ok前面去
        gbmap.transform() # 对地图进行改造 放到init ok前面去

        planner = Path_Planner(gbmap.map_info) # 放到init ok前面去
        ...
        for zhen in range(1, 15001):
            ...
            if now_frame_id == 1:
                update_reachable_region(robot, gbmap.map_info) # 放到init ok前面去
            ...
            end_t = time.time()
            exexution_time = end_t - start_t
            # sys.stderr.write(f"exexution_time: {exexution_time}\n")
            # sys.stderr.flush()
            
            if exexution_time < 0.013:
                time.sleep(0.013-exexution_time) # 更精确一点？ 取决于前面的计算花了多少时间
